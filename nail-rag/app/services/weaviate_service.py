...
class WeaviateService:
    ...
    async def _create_collection(self, client, collection_name: str) -> None:
        ...
        try:
            ...
            # Check if collection already exists - handle both sync and async
            try:
                ...
                # If it's not awaitable, it's synchronous
                exists = client.collections.exists(collection_name)
            ...
            # In Weaviate v4, properties are specified in the Property definition, not in vectorizer config
            # collections.create() may be async, so handle both cases
            ...
            # If create() returns a coroutine, await it
    ...
    async def ensure_collections_exist(self) -> bool:
        ...
        try:
            ...
            for collection_name in collections:
                # collections.exists() - check if it's async or sync
                try:
                    exists = client.collections.exists(collection_name)
                    # If it's a coroutine, await it
                    if hasattr(exists, '__await__'):
    ...
    async def check_document_exists(self, document_id: int, collection_name: str) -> bool:
        ...
        try:
            ...
            # Query by document_id - use data.get() for async operations in Weaviate v4
            result = await collection.data.get(
    ...
    async def _search_collection(
        ...
        try:
            ...
            # Hybrid search (vector + BM25) - async in Weaviate v4
